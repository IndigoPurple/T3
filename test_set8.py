...
data_list = ['tractor', 'motorbike', 'sunflower', 'park_joy', 'snowboard', 'touchdown', 'rafting', 'hypersmooth']

# log_location = './save/set8_iter/out_file.out'
...
for data in data_list:
   print('---------------',data)
   opt.input = './input/set8/'+ data +'/gray.mp4'
   opt.reference_dir = './input/set8/' + data +'/references/'

   print('Processing %s...'%os.path.basename(opt.input))

   outputdir = './save/set8_iter/%s/'%data
   ...
   #####  ref processing
   ref_ = refs[0].resize((t_w,t_h))
   ref_l, ref_ab = utils.convertRGB2LABTensor(ref_)

   # deep video prior
   max_iter=20
   lr1 = 1e-4
   optimizer_l = torch.optim.Adam(modelR.parameters(), lr=lr1)
   optimizer_ab = torch.optim.Adam(modelC.parameters(), lr=lr1)
   print("Start training prior with lr: %0.8f" % lr1)
   ...
   for iter in range(max_iter):
      ...
      out_l = output_l[0, :, 0, :, :]
      out_ab = output_ab[0, :, 0, :, :]
      ref_l, ref_ab = ref_l.to(device), ref_ab.to(device)

      mse = torch.nn.MSELoss()
      # print(out_l.shape, ref_l.shape)
      loss_l = mse(out_l,ref_l)
      loss_l.backward()
      loss_ab = mse(out_ab,ref_ab)
      optimizer_l.step()
      optimizer_ab.step()
      ...
      print('step: ', iter, ', loss_l: %.8f' % loss_l.item(), ', loss_ab: %.8f' % loss_ab.item())
      ...
      #    output.save(outputdir_out + '%07d_iter_%04d.png' % (i,iter))
   ...
   time_avg = time_cost_sum / max_iter
